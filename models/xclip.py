    ...
    def cache_animal_text(self, text):
        ...
        with torch.no_grad():
            if self.cache_animal_text_features is None:
                self.cache_animal_text_features = self.encode_text(text)
        self.train()
        return self.cache_animal_text_features

    def forward(self, image, text, animal_text, animal_pred):
        b = image.shape[0]
        ...
        text_features = text_features.unsqueeze(0).expand(b, -1, -1) # 16, 958, 512
        
        animal_pred = animal_pred.unsqueeze(1)
...
def load(model_path, name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", 
         ...
         use_cache=True, prompts_alpha=1e-1, prompts_layers=3, mit_layers=1,
):
